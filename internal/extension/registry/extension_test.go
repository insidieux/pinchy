...
func (s *providerListLookupTestSuite) TestErrorProviderWasNotRegistered() {
	...
	s.Nil(provider)
	s.Error(err)
	s.EqualError(err, `registry provider with name "provider" was not registered`)
}
...
func (s *providerListRegisterTestSuite) TestErrorProviderHasBeenRegistered() {
	...
	*s.list = append(*s.list, beforeRegisterProvider)
	...
	s.Error(err)
	s.EqualError(err, `registry provider with name "provider" has been already registered`)
}
...
func (s *providerListRegisterTestSuite) TestErrorProviderHasFlagsWithoutRequiredPrefix() {
	...
	s.EqualError(
		err,
		fmt.Sprintf(
			`registry "%s" flags validation error: flag "%s" does not contain required prefix "%s"`,
			`provider`,
			`wrong.flag`,
			flagPrefix,
		),
...
func (s *providerListRegisterTestSuite) TestSuccess() {
	flags := pflag.NewFlagSet(`provider`, pflag.ExitOnError)
	flags.String(`registry.flag`, `value`, `usage`)
	s.provider.On(`Flags`).Return(flags)
...
func (s *registerTestSuite) TestErrorProviderHasFlagsWithoutRequiredPrefix() {
	...
	err := Register(`provider`, flags, nil)
	s.Error(err)
	s.EqualError(
		err,
		fmt.Sprintf(
...
func (_m *MockFactory) Execute(_a0 *viper.Viper) (core.Registry, func(), error) {
	ret := _m.Called(_a0)

	var r0 core.Registry
	if rf, ok := ret.Get(0).(func(*viper.Viper) core.Registry); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(core.Registry)
		}
	}
